...
public:
	...
	/**
	* Moves to the next node. Returns false if no more nodes left.
	*/
	virtual bool next() = 0;
	...
	*/
	virtual irr::core::stringw getAttributeValue(const irr::core::stringw& name) = 0;

	/**
	* Returns value (as int) of the attribute with specified name.
	*/
	virtual int getAttributeValueAsInt(const irr::core::stringw& name) = 0;

	/**
	* Returns value (as float) of the attribute with specified name.
	*/
	virtual float getAttributeValueAsFloat(const irr::core::stringw& name) = 0;
